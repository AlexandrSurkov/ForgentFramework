...
|---|---|
| **Strong version control practices** | Feature branches, GitFlow, ADRs committed → `framework/spec/00-infrastructure.md` §0.5 |
| **Working in small batches** | Orchestrator decomposes: ≤1 day per subtask; hotfix branches → framework/spec/01-architecture.md §1.3 |
| **AI-accessible internal data** | AGENTS.md, llms.txt, SKILL.md, domain/ — machine-readable context → framework/spec/00-infrastructure.md §0.1 |
| **User-centric focus** | `.feature` scenarios describe real user flows; Phase 0 fixes the spec → framework/spec/01-architecture.md §1.3 |
| **Clear + communicated AI stance** | This spec is an explicit, documented AI stance |
| **Quality internal platform** | MCP servers, CI gates (Gate 1–3), observability pipeline → framework/spec/00-infrastructure.md §0.6; framework/spec/04-observability.md §4 |
| **Healthy data ecosystems** | JSONL traces, AI metrics, golden evals — feedback loop → framework/spec/04-observability.md §4; framework/spec/05-prompt-versioning.md §5 |
