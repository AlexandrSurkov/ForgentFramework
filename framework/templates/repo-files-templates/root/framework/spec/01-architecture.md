...
| `documentation-writer` | 0 (Component README), 6 | Phase 0: Component README ([00-infrastructure.md §0.7](00-infrastructure.md#07-readme-in-code)) with `architect`; Phase 6: API reference, How-to (Diataxis), updates to `.feature`, manual test plans |
...
| Subtask | Orchestrator during decomposition | a row from TASK_CONTEXT Decomposition | commit code/deliverables; return a `trace_event` JSON object in a `json` code block (no file writes; executor returns `trace_event`, orchestrator writes JSONL per `framework/spec/04-observability.md` §4.5–§4.6) |
...
```text
User adds ## Human Input to TASK_CONTEXT.md (template → [02-sessions-and-memory.md §2.3](02-sessions-and-memory.md#23-re-entry-protocol-after-needs_human)):
...
If NEEDS_HUMAN occurs a second time on the same subtask — ESCALATED (see [03-critic-rules-and-report-format.md §3.3](03-rubrics/03-critic-rules-and-report-format.md#33-mandatory-rules-for-all-critics)).
```

> The full `## Human Input` template and orchestrator details are in [02-sessions-and-memory.md §2.3](02-sessions-and-memory.md#23-re-entry-protocol-after-needs_human).
...
| — | Records a span `operation: "escalate"` in the JSONL trace (framework/spec/04-observability.md §4.6.2) |
...
| `agent-prompt-update` | Update to agent prompts / prompt procedures (see [05-prompt-versioning.md §5.3](05-prompt-versioning.md#53-agent-change-procedure)). |
...
| **Agent prompt update** (`fast_track: agent-prompt-update`) | `feature/*` | [05-prompt-versioning.md §5.3](05-prompt-versioning.md#53-agent-change-procedure) → CI Gate 1 | 0–6 pipeline |
