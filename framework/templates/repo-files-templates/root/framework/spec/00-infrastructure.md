...
│   │   └── awesome-copilot-gate.md    ← required when editing .github/agents/** or .github/prompts/** (see [07-framework-operations.md §7.3](07-framework-operations.md#73-awesome-copilot-gate-deterministic))
...
Traces are written locally to `.agents/traces/` (see `framework/spec/04-observability.md` §4).
...
> It is created and updated by an agent on every release commit (see [03-rubrics/11-dor-dod-and-adr-format.md](03-rubrics/11-dor-dod-and-adr-format.md) §3.11).
> Prompt changelog (`.github/AGENTS_CHANGELOG.md`) is a separate file in AgentConfig (see [05-prompt-versioning.md](05-prompt-versioning.md) §5.1).
...
model: <model-name>          # see PROJECT.md §2 + tiers in framework/spec/01-architecture.md §1.2.3
tools:
   # Canonical tool IDs: see framework/spec/04-observability.md §4.8
...
## Role
[Role: executor or critic; responsibility zone; pipeline phases from framework/spec/01-architecture.md §1.3]
...
- Follow all principles from framework/spec/03-rubrics/00-constitutional-principles.md §3.0
...
Trace-writing protocol and `trace_event` required keys are defined in framework/spec/04-observability.md §4.5–§4.6.
...
from framework/spec/03-rubrics/04-backend-critic.md §3.4 and project-specific triggers from PROJECT.md §4.
...
2. For each file: apply Backend Critic checklist (framework/spec/03-rubrics/04-backend-critic.md §3.4 + PROJECT.md §4 triggers).
3. Check Constitutional principles (framework/spec/03-rubrics/00-constitutional-principles.md §3.0): zone violations, hardcoded config, skipped tests.
...
## Constitutional Constraints
- Follow framework/spec/03-rubrics/00-constitutional-principles.md §3.0 and framework/spec/03-rubrics/03-critic-rules-and-report-format.md §3.3 (all critic rules).
...
### Trace Recording
After issuing the verdict, return a `trace_event` object as specified by framework/spec/04-observability.md §4.5–§4.6.
````
...
[See framework/spec/00-infrastructure.md §0.5 GitFlow + SemVer — detailed rules]
...
> For the Frontend repo, additionally: `docs/test-plans/` — manual test plans (`framework/spec/01-architecture.md` §1.4).
...
<!-- The baseline agent set is defined in framework/spec/01-architecture.md §1.
...
<!-- Fill during setup. Tiers T1/T2/T3 are defined in framework/spec/01-architecture.md §1.2.3.
...
<!-- Universal model selection rules are in framework/spec/01-architecture.md §1.2.
       Record only deviations here: budget constraints, provider policy, missing models.
   Orchestrator reads this section and applies it with higher priority than framework/spec/01-architecture.md §1.2.
...
> If there are no custom rules — delete the table above. Everything else is governed by the lines above + framework/spec/01-architecture.md §1.2.
...
## 4. Technology-specific rubric triggers (add-on to framework/spec/03-rubrics/* universal rubrics)
...
> Concrete servers configured in `.vscode/mcp.json`. Example structure — framework/spec/06-adoption-roadmap.md §6.4.
...
See: framework/00-multi-agent-development-spec.md for full protocol.
