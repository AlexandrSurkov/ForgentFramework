...
| **`mcp.json`** | VS Code workspace, 2024 | Workspace-level MCP server configuration. Declares which MCP servers are available to agents in the workspace (see [00-infrastructure.md §0.1](../00-infrastructure.md#01-agentconfig-repository-structure)). |
...
| Standard | Source | Where it is used |
|---|---|---|
| **promptfoo** | [promptfoo.dev](https://promptfoo.dev), 2023 | Evaluation framework for LLMs and agents: golden tests, regression suites, YAML-based configs. Used directly in `.agents/evals/` for per-agent golden tests ([06-adoption-roadmap.md §6.7](../06-adoption-roadmap.md#67-phase-7--evals-optional)). |
| **HELM** | Liang et al., Stanford CRFM, 2022 | Holistic Evaluation of Language Models: scenario coverage, accuracy / calibration / fairness / robustness metrics. Academic basis for eval suite design and metric selection in [06-adoption-roadmap.md §6.7](../06-adoption-roadmap.md#67-phase-7--evals-optional). |
...
| **AgentBench** | Liu et al., Tsinghua, 2023 | Benchmark for evaluating LLMs as agents across diverse environments (OS, DB, web, code). Academic basis for designing multi-environment eval scenarios in [06-adoption-roadmap.md §6.7](../06-adoption-roadmap.md#67-phase-7--evals-optional); defines scenario coverage expectations. |
...
| **Supply-chain discipline for tools** — new MCP servers/tools are treated as a supply-chain event; deviations from defaults require an ADR. | A1.3 (OWASP LLM06), A1.1 (MCP, ADR) | [agent-file-standards SKILL](../../../.agents/skills/agent-file-standards/SKILL.md), [ai-security SKILL](../../../.agents/skills/ai-security/SKILL.md), [.github/decisions/](../../../.github/decisions/) |
| **Prefer open protocols over bespoke glue** — use MCP for tool access, A2A for agent-to-agent communication, and OTel GenAI conventions for traces/telemetry. | A1.1 | `.vscode/mcp.json`, `.agents/traces/README.md` |
| **Eval-before-merge** — if a project adopts golden tests, they must pass before merging any agent prompt / rubric change. | A1.5 (promptfoo), A1.6 | See A1.5 and [06-adoption-roadmap.md §6.7](../06-adoption-roadmap.md#67-phase-7--evals-optional) |
| **Measurement over vibes** — evaluation should measure *coverage* and *quality dimensions* (not only “does it work”). | A1.5 (HELM, G-Eval, AgentBench) | See A1.5 and [06-adoption-roadmap.md §6.7](../06-adoption-roadmap.md#67-phase-7--evals-optional) |
| **RAG is evaluated as a system** — when retrieval is involved, evaluate faithfulness and context precision/recall (not only answer fluency). | A1.5 (RAGAS), A1.2 (RAG) | See A1.5 and [06-adoption-roadmap.md §6.7](../06-adoption-roadmap.md#67-phase-7--evals-optional) |
