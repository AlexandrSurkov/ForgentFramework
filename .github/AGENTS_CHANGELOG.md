...
| 2026-03-01 | `forgent-docs-critic`, `forgent-process-critic` | tools/behavior | Restored least-privilege for critics (read-only tools); require deterministic finding locations (file + exact heading; file+line when available). | copilot |
| 2026-03-01 | `forgent-spec-editor`, `forgent-process-critic` | behavior | Deterministically encode AWESOME-COPILOT gate: spec-editor must run gate and ensure the canonical report exists at `.agents/compliance/awesome-copilot-gate.md` when triggered; defined Minimal schema v1 for the report; process-critic blocks if report missing or fails the schema checklist. | copilot |
| 2026-03-01 | `forgent-spec-editor` | behavior | Tightened AWESOME-COPILOT Minimal schema v1 to be deterministic: required fields must be filled and the report must be updated alongside the agent/prompt edits (aligns with the process-critic completeness checklist). | copilot |
| 2026-03-01 | `forgent-process-critic` | fix | Clarified AWESOME-COPILOT completeness checklist and failure conditions (missing/stale/incomplete canonical report yields BLOCKER). | copilot |
| 2026-03-01 | `forgent-spec-editor`, `forgent-process-critic` | behavior | Aligned AWESOME-COPILOT gate to the canonical, non-session artifact `.agents/compliance/awesome-copilot-gate.md` (Operations §7.3). Removed the session-scoped gate report path; process-critic now blocks on missing/stale/incomplete canonical gate report. | copilot |
| 2026-03-01 | `forgent-spec-editor`, `forgent-process-critic` | behavior | Reinforced AWESOME-COPILOT gate determinism: explicitly states `.agents/compliance/awesome-copilot-gate.md` is the only allowed gate report location, and enumerates missing/incomplete canonical gate report as explicit BLOCKER failure conditions when the trigger fires. | copilot |
| 2026-03-01 | `forgent-spec-editor` | behavior | Added explicit tool-call protocol requirement: pre-call why/what/outcome + post-call observation/next-step for each tool invocation. | copilot |
| 2026-03-01 | `forgent-docs-critic` | behavior | Mode B (audit executor) now requires a brief pre-tool-call justification stating why the tool call is needed. | copilot |
